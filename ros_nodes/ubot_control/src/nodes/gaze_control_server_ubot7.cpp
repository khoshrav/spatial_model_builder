...
public:
    ...
    GazeController(ros::NodeHandle &n, std::string name) :
        ...
        ROS_INFO("KDL JOINTS = %d", kdl_chain_.getNrOfJoints());
        ...
        nJoints_ = kdl_chain_.getNrOfJoints();
        loadJointLimits(joint_limit_min_, joint_limit_max_);

        gaze_.Initialize(kdl_chain_, joint_limit_min_, joint_limit_max_, 20);
    ...
    void jointPositionCallback(const ubot_msgs::JointPositionsConstPtr &msg) {
        ...
        {
          joint_positions_(i) = msg->positions[HEAD_TILT + i];
        }
        // joint_positions_(0) = msg->positions[HEAD_TILT];
        // joint_positions_(1) = msg->positions[HEAD_TILT + 1];
        // joint_positions_(2) = msg->positions[HEAD_TILT + 2];
    ...
    void jointDesiredPositionCallback(const ubot_msgs::JointPositionsConstPtr &msg) {
        ...
        for (size_t i = 0; i < nJoints_; i++)
        {
          desired_joint_positions_(i) = msg->positions[HEAD_TILT + i];
        }
        // desired_joint_positions_(0) = msg->positions[HEAD_TILT];
        // desired_joint_positions_(1) = msg->positions[HEAD_TILT + 1];
        // desired_joint_positions_(2) = msg->positions[HEAD_TILT + 2];
    ...
    void preemptCB() {
      ...
      void run() {
        ...
        if (state_ == NODE_NO_REFERENCE) {
            ...
            joint_pos.positions.resize(14);
            joint_pos.numJoints = 14;
            joint_pos.positions[11] = head_jnt_pos(0);
            joint_pos.positions[12] = head_jnt_pos(1);
            joint_pos.positions[13] = head_jnt_pos(2);
            // joint_pos.positions[HEAD_TILT] = head_jnt_pos(0);

            // TODO: CHECK BITMASK
            joint_pos.bitmask |= 14336;
            // publish new positions
            joint_pos_pub_.publish(joint_pos);
            for (int i = 0; i < nJoints_; i++)
            {
              ROS_INFO("head_jnt_pos[%d] = %f", i, head_jnt_pos(i));
            }
